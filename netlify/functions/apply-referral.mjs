...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!newUserId || !refCode) {
      return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Missing newUserId or refCode' }) };
    }
    ...
    if (refCode.toUpperCase() === ownCode) {
      return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Cannot refer yourself' }) };
    }
    ...
    if (findError || !referralRow) {
      return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'Referral code not found' }) };
    }
    ...
    if (alreadyReferred) {
      return { statusCode: 409, headers: CORS, body: JSON.stringify({ error: 'Referral already applied' }) };
    }
