...
export const handler = async (event) => {
  ...
  if (!RESEND_API_KEY) {
    ...
    return { statusCode: 200, headers: CORS, body: JSON.stringify({ sent: false, reason: 'no_key' }) };
  }
  ...
  try {
    ...
    if (!to || !type) {
      return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Missing to or type' }) };
    }

    const templates = { welcome: welcomeEmail, limit_reached: limitReachedEmail, payment_success: paymentSuccessEmail };
    const template = templates[type];
    if (!template) {
      return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: `Unknown email type: ${type}` }) };
    }
