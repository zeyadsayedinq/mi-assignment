...
export const handler = async (event) => {
  ...
  if (!TAP_SECRET_KEY) {
    ...
    return {
      statusCode: 500,
      headers: CORS,
      body: JSON.stringify({ error: 'Payment service not configured. Contact support.' }),
    };
  }

  try {
    const { plan, userId, email, currency = 'EGP' } = JSON.parse(event.body || '{}');

    if (!plan || !userId || !email) {
      return {
        statusCode: 400,
        headers: CORS,
        body: JSON.stringify({ error: 'Missing required fields: plan, userId, email' }),
      };
    }
    ...
    if (!['EGP', 'SAR', 'AED'].includes(currency)) {
      return {
        statusCode: 400,
        headers: CORS,
        body: JSON.stringify({ error: `Invalid currency: ${currency}` }),
      };
    }
    ...
    return {
      statusCode: 500,
      headers: CORS,
      body: JSON.stringify({ error: e.message || 'Payment failed. Please try again.' }),
    };
  }
};
