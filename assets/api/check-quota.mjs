...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!userId) { Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(401).json({ error: 'Unauthorized' }); }
    ...
    if (email && OWNER_EMAILS.includes(email.toLowerCase())) {
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(200).json({ allowed: true, plan: 'owner', limit: 999999 });
    }
    ...
    if (!canUse) {
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(402).send(JSON.stringify({
          error: 'LIMIT_REACHED',
          plan,
          missionsUsed,
    ...
    Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(200).json({ allowed: true });
  } catch (e) {
