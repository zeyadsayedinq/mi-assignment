...
export default async function handler(req, res) {
  ...
  if (!userId) Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(400).json({ error: 'Missing userId' });

  try {
    if (req.method === 'GET') {
      const { data: sub } = await supabase.from('subscriptions').select('*').eq('user_id', userId).single();
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(200).json({ subscription: sub });
    }

    if (req.method === 'POST') {
      const { plan, action, days } = req.body;
      ...
      if (action === 'revoke' || plan === 'free') {
        ...
        Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(200).json({ success: true, action: 'revoked' });
      }
      ...
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(200).json({ success: true, action: 'granted', plan });
    }
