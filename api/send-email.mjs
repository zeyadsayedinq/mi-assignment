...
export default async function handler(req, res) {
  ...
  if (!RESEND_API_KEY) {
    ...
    setCORS(res); return res.status(200).json({ sent: false, reason: 'no_key' });
  }
  ...
  try {
    ...
    if (!template) {
      setCORS(res); return res.status(400).json({ error: `Unknown email type: ${type}` });
    }
    ...
    setCORS(res); return res.status(200).json({ sent: true, id: result.id });
  } catch (e) {
