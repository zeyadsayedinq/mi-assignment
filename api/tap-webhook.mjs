...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!chargeId) {
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(400).json({ error: 'No charge ID in webhook' });
    }
    ...
    if (charge.status !== 'CAPTURED') {
      ...
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(200).json({ received: true, action: 'none' });
    }
    ...
    if (!userId || !plan) {
      ...
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(400).json({ error: 'Missing metadata' });
    }
    ...
    Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(200).send(JSON.stringify({ received: true, action: 'subscription_activated', plan, userId }),);
  } catch (e) {
    console.error('tap-webhook error:', e.message);
    // Return 200 to prevent Tap from retrying indefinitely for non-transient errors
    Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(200).send(JSON.stringify({ received: true, error: e.message }),);
  }
};
