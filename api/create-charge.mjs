...
export default async function handler(req, res) {
  ...
  if (!TAP_SECRET_KEY) {
    ...
    Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(500).send(JSON.stringify({ error: 'Payment service not configured. Contact support.' }),);
  }

  try {
    const { plan, userId, email, currency = 'EGP' } = req.body;

    if (!plan || !userId || !email) {
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(400).send(JSON.stringify({ error: 'Missing required fields: plan, userId, email' }),);
    }

    if (!PLAN_PRICES[plan]) {
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(400).send(JSON.stringify({ error: `Invalid plan: ${plan}` }),);
    }

    if (!['EGP', 'SAR', 'AED'].includes(currency)) {
      Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(400).send(JSON.stringify({ error: `Invalid currency: ${currency}` }),);
    }
    ...
    Object.entries(CORS).forEach(([k,v]) => res.setHeader(k,v)); return res.status(500).send(JSON.stringify({ error: e.message || 'Payment failed. Please try again.' }),);
  }
};
