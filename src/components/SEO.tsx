...
import { getUniversityBySlug, UNIVERSITY_PAGES } from '../data/universityPages';
...
const PAGE_META: Record<string, { title: string; description: string; keywords?: string }> = {
  ...
  '/universities': {
    title: 'حل الواجبات حسب جامعتك — Mi-Assignment | Assignment Help by University',
    description: 'حل واجبات مُعاير على معايير جامعتك: GUC، AUC، القاهرة، الملك سعود، KFUPM، الإمارات، AUB وغيرها. أكواد هندسية وأنظمة توثيق لكل جامعة.',
    keywords: 'حل واجبات جامعية, assignment help by university, GUC, AUC, KSU, KFUPM, UAEU, AUB, حل واجب جامعتي',
  },
  '/terminal': {
...
// Resolves meta for dynamic routes (e.g. /universities/:slug)
function resolveMeta(pathname: string): { title: string; description: string; keywords?: string } {
  if (pathname.startsWith('/universities/')) {
    const slug = pathname.replace('/universities/', '').replace(/\/$/, '');
    const uni = getUniversityBySlug(slug);
    if (uni) {
      return { title: uni.metaTitle, description: uni.metaDescription, keywords: uni.keywords };
    }
  }
...
export function SEO({ title, description, canonical, noindex = false }: SEOProps) {
  ...
  }, [finalTitle, finalDesc, finalCanonical, noindex, meta.keywords]);
